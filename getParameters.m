...
%%
        ...
        P.connection_density = [];          % [0,1]; probabilistic proportion of existing weights to all possible weights; 0 to 1
        %P.connections_per_neuron = round(P.nbof_neurons/2); % the exact number of connections per neuron for each neuron
        P.activation_function = @transferfn_threshold_linear;   % @transferfn_threshold_linear (0, Inf), @transferfn_@transferfn_tanh (-1/+1), @transferfn_step (0/1)
