...
%%
        ...
        % Initialization
        P.weight_init = @randn;             % @zeros, @rand, @ones (@randn can be negative); with Hebbian2 + @zeros the network does not learn!
        P.strenght_of_memory_traces = 0.1;    % multiplier of the initial weights
        ...
        P.forgetting_rate = 0.1;            % weights are multiplied by this number before each trainig session
        ...
        P.noise = 0;                        % size of noise on input during testing
...
%%
        ...
        % Architecture
        P.nbof_neurons = 50;                % number of neurons: 1000
        P.connection_density = [0.3];       % [0,1] or []; probabilistic proportion of existing weights to all possible weights; if it's empty, then P.connections_per_neuron is used instead
        P.connections_per_neuron = [];      % the exact number of connections per neuron for each neuron
        P.activation_function = @transferfn_step;   % @transferfn_threshold_linear (0, Inf), @transferfn_@transferfn_tanh (-1/+1), @transferfn_step (0/1)
        P.gain_factor = 0.5;                % slope of the threshold linear activation function
        P.threshold = 0;                    % activation threshold for @transferfn_step and @transferfn_threshold_linear; starting value when autoupdate enabled
        P.allow_selfloops = 0;              % 0/1; whether to allow self-loops
        ...
        P.learning_rule = 'covariance2';    % 'Hebbian1', 'Hebbian2', 'covariance1', 'covariance2' (see TrainAttractor)
        P.learning_rate = 1;                % learning rate
        P.forgetting_rate = 0.5;            % weights are multiplied by this number before each trainig session
        P.autothreshold_aftertraining = 0;  % 0/1; enables automatic threshold after training
        P.sparseness_difference = 0.001;    % maximum allowable difference between input and output sparseness wehn setting threshold
        P.threshold_incr = 0.0001;          % the increment with which to change the threshold during threshold setting
        P.threshold_setting_timeout = 100;  % maximum number of steps when setting the threshold
        ...
        P.autothreshold_duringtesting = 1;  % 0/1; automatically set thresholds separately for each neuron after each recurrent cycle during testing
        P.noise = 0;                        % size of noise on input during testing
        P.missing_perc = 100;                 % the percentage of missing input bits during testing        
        %%
    otherwise
